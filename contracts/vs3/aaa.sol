...
contract ASTERDAO is ERC20, Ownable, ReentrancyGuard {
    ...
    function unstakeLP() external {
        ...
        if (isEarlySpecial) {
            _handleEarlyUnstake(msg.sender, amount);      // 100天内特殊用户
            emit LPUnstaked(msg.sender, amount, true);
        } else {
            _handleFullUnstake(msg.sender, amount);       // 普通用户 或 >=100天
    ...
    function pendingReward(address user) public view returns (uint256) {
        ...
        // 扣除 LP 10% + NFT 2% + 推荐链 10%
        uint256 lpShare = (grossReward * 10) / 100;
        uint256 nftShare = (grossReward * 2) / 100;
        uint256 referralShare = (grossReward * 10) / 100;

        uint256 userNet = grossReward - lpShare - nftShare - referralShare;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount == 1 * 10**18 && stakes[from].active) {
            ...
            if (reward > 0) {
                ...
                super._transfer(address(this), from, reward);
    ...
    function _addLiquidityFromTax(uint256 tokenAmount) internal {
        if (tokenAmount == 0 || pairAddress == address(0)) return;

        uint256 half = tokenAmount / 2;
        uint256 otherHalf = tokenAmount - half;

        // 1. 把一半 ASTERDAO 换成 WBNB
        _approve(address(this), PANCAKE_ROUTER, half);
        ...
        // 2. 添加流动性（创建底池）
        _approve(address(this), PANCAKE_ROUTER, otherHalf);
        IERC20(WBNB).approve(PANCAKE_ROUTER, wbnbAmount);

        try IUniswapV2Router02(PANCAKE_ROUTER).addLiquidity(
            address(this),           // tokenA = ASTERDAO
            WBNB,                    // tokenB = WBNB
            otherHalf,
            wbnbAmount,
            0,                       // amountAMin
            0,                       // amountBMin
            lpPoolAddress,           // LP Token 接收地址（建议设为死地址或锁仓合约）
            block.timestamp + 300
    ...
    require(tokenAmount > 0 && balanceOf(address(this)) >= tokenAmount, "insufficient");

    // 只 swap 很小比例（比如 5%）去换 WBNB，其余 95% ASTERDAO 直接进底池
    uint256 swapPercent = 5; // 可调，5% 换 WBNB，95% 直接进底池
    uint256 amountToSwap = (tokenAmount * swapPercent) / 100;
    uint256 amountToAddDirectly = tokenAmount - amountToSwap;

    // 1. 把小部分 ASTERDAO 换成 WBNB
    if (amountToSwap > 0) {
        ...
        uint256[] memory amounts = IUniswapV2Router02(PANCAKE_ROUTER)
        ...
        // 2. 用换来的少量 WBNB + 大量 ASTERDAO 添加流动性
