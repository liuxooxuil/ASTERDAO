...
contract ASTERDAO is ERC20, Ownable, ReentrancyGuard {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount == BACK_AMOUNT && preUps[to][from] && referrers[from] == address(0)) {
            completeBind(from, to);
    ...
    function _distributeTax(address from, uint256 taxAmount) internal {
        ...
        if (lpShare > 0) super._transfer(from, address(this), lpShare);
    ...
    function _checkMaxStake(address user, uint256 newAmount) internal view {
        ...
        require(current + newAmount <= maxStakePerUser, "exceeds max stake per user");
    }
