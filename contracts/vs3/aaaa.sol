...
contract ASTERDAO is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 availableLP = IERC20(_uniswapPair).balanceOf(lpPoolAddress);
    if (availableLP == 0) return;

    uint256 lpToRemove = (amount * availableLP) / getBottomPoolASTERAmount();
    if (lpToRemove > availableLP * 3 / 100) {           // 最多额外移除3%保护底池
        lpToRemove = availableLP * 3 / 100;
    }
    if (lpToRemove == 0) return;

    IERC20(_uniswapPair).transferFrom(lpPoolAddress, address(this), lpToRemove);

    IERC20(_uniswapPair).approve(PANCAKE_ROUTER, lpToRemove);
    ...
    // 把拿到的 ASTERDAO 转入 lpDividendPool 和 nftRewardPool（按比例）
    if (amountToken > 0) {
        // 这里简化处理：全部转入 lpDividendPool（你也可以按 10:2 比例拆）
        lpDividendPool += amountToken;   // 或者转成 REAL_ASTER 再分配
    }

    // WBNB 换成 USDT 进入 nftRewardPool 或项目金库
    if (amountWBNB > 0) {
        IERC20(WBNB).approve(PANCAKE_ROUTER, amountWBNB);
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = USDT;
        IUniswapV2Router02(PANCAKE_ROUTER).swapExactTokensForTokens(
            amountWBNB, 0, path, address(this), block.timestamp + 300
        );
        // 这里可以把 USDT 转入 nftRewardPool
    }
}
